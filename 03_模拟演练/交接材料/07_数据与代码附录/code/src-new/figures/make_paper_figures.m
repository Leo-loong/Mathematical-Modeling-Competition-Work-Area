...
% ================= 图5: 合成闭环 + bootstrap 分布 (数据源 V4_results.mat) =================
matFile = fullfile(rootDir, '模型设计', '实验包-1', 'work', 'out', 'V4_results.mat');
if exist(matFile, 'file')
    S = load(matFile);
    h = figure('Visible', 'off');
    subplot(1, 2, 1);
    bar(S.d0list, S.spread'); grid on;
    set(gca, 'XTickLabel', string(S.d0list), 'FontName', 'Arial');
    xlabel('设定真厚度 d_0 (\mum)'); ylabel('稳健离散 spread (\mum)');
    legend({'E1', 'E2', 'E3'}, 'Location', 'northwest');
    title('合成闭环: 算法自身精度上限');
    subplot(1, 2, 2);
    histogram(S.fusedBoot(1, :), 20, 'FaceAlpha', 0.6, 'EdgeColor', 'none'); hold on;
    histogram(S.fusedBoot(2, :), 20, 'FaceAlpha', 0.6, 'EdgeColor', 'none');
    xline(S.dFinalEmp, 'k-', 'LineWidth', 1.0);
    grid on; legend({'10°', '15°'}, 'Location', 'best');
    xlabel('融合 d (\mum)'); title('融合厚度 bootstrap 分布 (N=200)');
    paperfig_style(h); paperfig_export(h, 'fig5_closedloop', figDir);
    T5s = table(S.d0list', S.spread(:,1), S.spread(:,2), S.spread(:,3), ...
        'VariableNames', {'d0_um', 'E1', 'E2', 'E3'});
    writetable(T5s, fullfile(dataDir, 'fig5_spread.csv'));
    T5b = table(S.fusedBoot(1, :)', S.fusedBoot(2, :)', ...
        'VariableNames', {'fused10', 'fused15'});
    writetable(T5b, fullfile(dataDir, 'fig5_boot.csv'));
else
    fprintf('SKIP fig5: V4_results.mat 不存在 (先重跑 run_v15_pipeline)\n');
end
