...
%                  quiet. ([1e-6 1 0])
%   termFN       - name of the .m termination function (['maxGenT'])
%   termOps      - options string to be passed to the termination function
%                  ([100]).
%   selectFN     - name of the .m selection function (['normGeoS'])
%   selectOpts   - options string to be passed to select after
%                  select(pop,#,opts) ([0.08])
%   xOverFNS     - a string containing blank seperated names of Xover.m
%                  files (['arithX heuristX simpleX']) 
%   xOverOps     - A matrix of options to pass to Xover.m files with the
%                  first column being the number of that xOver to perform
%                  similiarly for mutation ([2 0;2 3;2 0])
%   mutFNs       - a string containing blank seperated names of mutation.m 
%                  files (['boundM multiNUM ...
%                           nonUnifM unifM'])
...
if n<12 %Default muatation information
  if opts(2)==1 %Float GA
  mutFNs=['boundM multiNUM nonUnifM unifM'];
...
if n<10 %Default crossover information
  if opts(2)==1 %Float GA
    xOverFNs=['arithX heuristX simpleX'];
...
if startPop==[] %Generate a population at random
  ...
  startPop=initial(80,bounds,evalFN,evalOps,opts(1:2));
end
