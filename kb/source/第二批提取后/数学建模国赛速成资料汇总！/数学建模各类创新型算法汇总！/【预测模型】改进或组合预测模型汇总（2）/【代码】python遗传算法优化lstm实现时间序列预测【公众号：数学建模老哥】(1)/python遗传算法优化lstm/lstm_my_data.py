...
def aim_function(x_train, y_train, x_test, y_test, num):
    ...
    # 这里是最后一层：分类层，softmax
    outputs_lstm = Dense(1, activation='relu')(lstm_dense_batch_name[lstm_dense_layers - 1])
    ...
    history = LSTM_model.fit(x_train, y_train, batch_size=32, epochs=100, validation_split=0.1, verbose=1)
    # 验证模型,model.evaluate返回的值是一个数组，其中score[0]为loss,score[1]为准确度
    mae = LSTM_model.evaluate(x_test, y_test, verbose=0)
    # In[]计算各种指标
    test_pred = LSTM_model.predict(x_test)
    test_pred1 = test_pred.reshape(-1, 1)
    test_label1 = y_test.reshape(-1, 1)
    ...
    plt.legend()
    plt.savefig('lstm_layers_'+str(lstm_layers)+'_lstm_units_'+str(lstm_units)+'.png')
    plt.show()
    return mae[1]
