...
int set_matrix(lprec *lp, int Row, int Column, REAL Value, MYBOOL doscale)
{
  ...
  if(Row > lp->rows || Row < 0) {
    report(lp, SEVERE, "set_matrix: Row %d out of range", Row);
    return(FALSE);
  }
  if(Column > lp->columns || Column < 1) {
    report(lp, SEVERE, "set_matrix: Column %d out of range", Column);
    return(FALSE);
  }

  /* scaling is performed twice? MB -- see below! KE */
/*
...
*/
  ...
  while((elmnr < lp->col_end[Column]) && (lp->mat[elmnr].row_nr /* < */ != Row)) /* changed from != to < to make sure data is ordened, but lag_solve then hangs ... <peno> */
    elmnr++;
...
{
  int i, j, k, from_elm, to_elm, elm_in_col;
  if(column > lp->columns || column < 1) {
...
void set_upbo(lprec *lp, int column, REAL value)
{
  ...
  if(value < lp->orig_upbo[lp->rows + column]) { /* is this correct ? <peno> */
    lp->eta_valid = FALSE;
...
void set_lowbo(lprec *lp, int column, REAL value)
{
  ...
  if((value < 0) || (value > lp->orig_lowbo[lp->rows + column])) /* is this correct ? <peno> */
  {
    lp->eta_valid = FALSE;
...
short is_int(lprec *lp, int column)
{
  ...
  return((short) ((lp->must_be_int[column] & ISINTEGER) != 0));
}
...
short is_semicont(lprec *lp, int column)
{
  ...
  return((short) ((lp->must_be_int[column] & ISSEMI) != 0));
}
...
short is_SOS_var(lprec *lp, int column)
{
  ...
  return((short) ((lp->must_be_int[column] & ISSOS) != 0));
}
...
short SOS_get_type(lprec *lp, int sosindex)
{
...
short SOS_is_member(lprec *lp, int sosindex, int column)
{
  int i;
  short n;
  int *list;
...
/* The return code is [-2..+2], depending on the type of satisfaction */
{
  ...
  if(sosindex == 0) {
    for(i = 1; i<=lp->sos_count; i++) {
      status = SOS_is_satisfied(lp, i, solution);
      if(status != 0 && status != -1) break;
    }
  }
  else
  {
    type = SOS_get_type(lp, sosindex);
    list = lp->sos_list[sosindex-1]->members;
    n = list[0]+1;
    nn = list[n];
...
int find_mat_elm(lprec *lp, int row, int column)
{
...
REAL get_mat_raw(lprec *lp, int row, int column)
{
  ...
  elmnr = find_mat_elm(lp, row, column);
  if(elmnr >= 0)
...
static void mult_column(lprec *lp, int col_nr, REAL mult)
{
  int i;

  if(col_nr < 1 || col_nr > lp->columns) {
    report(lp, IMPORTANT, "multi_column: Column %d out of range", col_nr);
    return;
  }
  for(i = lp->col_end[col_nr - 1]; i < lp->col_end[col_nr]; i++)
    lp->mat[i].value *= mult;
