	   ...
	   REAL *pcol)
{
  ...
  for(i = start; i <= end; i++)
      ...
      if(theta != 0)
	{
	  ...
	  for(rowp = Eta_row_nr + j, valuep = Eta_value + j;
	      j < k;
	      j++, rowp++, valuep++)
...
{
  Eta_alloc *= 1.5;
  REALLOC(Eta_value, Eta_alloc);
  Lp->eta_value = Eta_value;
  REALLOC(Eta_row_nr, Eta_alloc);
...
void invert(void)
{
  ...
  if(Lp->print_at_invert) 
    fprintf(stderr, "Start Invert iter %7d eta_size %4d rhs[0] %16.4f \n",
	    Lp->iter, Eta_size, (double) - Rhs[0]); 
 
  CALLOC(rownum, Rows + 1);
  CALLOC(col, Rows + 1);
  CALLOC(row, Rows + 1);
  CALLOC(pcol, Rows + 1);
  CALLOC(frow, Rows + 1);
  CALLOC(fcol, Columns + 1);
  CALLOC(colnum, Columns + 1);
		     ...
		     REAL *pcol)
{
  ...
  for(i = 1; i <= Rows; i++)
      ...
      if(f != 0)
	{
	  if(my_abs(f) < Trej)
	    {
	      debug_print("pivot %g rejected, too small (limit %g)\n",
  ...
  if((*theta) < 0)
    {
      ...
      fprintf(stderr, "pcol[%d] = % 18.10f, rhs[%d] = % 18.8f , upbo = % f\n",
	      (*row_nr), (double)f, (*row_nr), (double)Rhs[(*row_nr)],
	      (double)Upbo[Bas[(*row_nr)]]);
    }
  if((*row_nr) == 0)
    {
      ...
      if((*minit))
        {
          if(!Lower[varin])
		    ...
		    Lp->iter, varin, (double)Upbo[varin]);
          else
            fprintf(stderr,
		...
		Lp->iter, varin, (double)Rhs[row_nr]);
      if(!primal)
	{
	  f = 0;
	  for(i = 1; i <= Rows; i++)
...
static void check_if_less(REAL x, REAL y, REAL value)
{
  if(x >= y)
    {
      fprintf(stderr,
...
static void check_solution(REAL *sol, REAL *upbo, REAL *lowbo)
{
  int i;
...
#define CHECK_EPS 1e-2
  ...
  if(Lp->columns_scaled)
    for(i = Rows + 1; i <= Sum; i++)
      {
	if(sol[i] < lowbo[i] * Lp->scale[i] - CHECK_EPS)
	  {
	    fprintf(stderr,
		    "Error: variable %d (%s) has a solution (%g) smaller than its lower bound (%g)\n",
		    i - Rows, Lp->col_name[i - Rows], sol[i],
		    lowbo[i] * Lp->scale[i]);
	    /* abort(); */
	  }
	if(sol[i] > upbo[i] * Lp->scale[i] + CHECK_EPS)
	  {
	    fprintf(stderr,
    ...
    for(i = Rows + 1; i <= Sum; i++)
      {
	if(sol[i] < lowbo[i] - CHECK_EPS)
	  {
	    fprintf(stderr,
		    "Error: variable %d (%s) has a solution (%g) smaller than its lower bound (%g)\n",
		    i - Rows, Lp->col_name[i - Rows], sol[i], lowbo[i]);
	    /* abort(); */
	  }
	if(sol[i] > upbo[i] + CHECK_EPS)
	  {
	    fprintf(stderr,
		    "Error: variable %d (%s) has a solution (%g) larger than its upper bound (%g)\n",
		    i - Rows, Lp->col_name[i - Rows], sol[i], upbo[i]);
	    /* abort(); */
	  }
      }
}
		     ...
		     int     recursive)
{
  ...
  /* make shure we do not do memcpy(Basis, Basis ...) ! */
  if(recursive)
    {
  ...
  if(failure == OPTIMAL) /* there is a good solution */
    {
      ...
      /* check_solution(Solution, upbo, lowbo); get too many hits ?? */
      ...
      if(is_worse)
	{
	  if(Lp->verbose)
	    fprintf(stderr, "level%4d OPT NOB value %g bound %g\n",
		    Level, (double)Solution[0], (double)Best_solution[0]); 
		...
		if(lowbo[i] == upbo[i]) /* this var is already fixed */
		  {
		    fprintf(stderr,
			    ...
			    i - Rows, (int)lowbo[i], (double)Solution[i]);
      ...
      if(notint) /* there is at least one value not yet int */
	{
	  ...
	  MALLOC(new_upbo,  Sum + 1);
	  MALLOC(new_lowbo, Sum + 1);
          MALLOC(new_lower, Sum + 1);
          MALLOC(new_basis, Sum + 1);
          MALLOC(new_bas,   Rows + 1);
	  ...
	  debug_print_bounds(upbo, lowbo);
          ...
          if(Floor_first)
	        ...
	        {
		  check_if_less(new_bound, upbo[notint], Solution[notint]);
		...
		{
		  check_if_less(lowbo[notint], new_bound, Solution[notint]);
