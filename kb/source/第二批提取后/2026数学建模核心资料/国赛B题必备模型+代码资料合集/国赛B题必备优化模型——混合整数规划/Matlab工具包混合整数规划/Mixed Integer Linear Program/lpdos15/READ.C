...
static int hashval(const char *string)
{
  ...
  for(; *string; string++)
	...
	{
	  /* if the any of the most significant bits is on */
...
{ 
  ...
  if(value == 0)
    {
      fprintf(stderr, "(store) Warning, variable %s has an effective coefficient of 0 on line %d. Ignored.\n", variable, yylineno);
      return;
    }
...
void store_re_op(void)
{
  ...
  if(Solve_dual) /* allocate new rside struct here */
    {
      CALLOC(rp, 1, rside);
      rp->next = First_rside;
      First_rside = rp;
  ...
  if((Lin_term_count > 1) || Solve_dual) /* it is not a bound */
...
{
  if((Lin_term_count > 1) || Solve_dual) /* not a bound */
	       ...
	       REAL value)
{
  ...
  /* always store objective function with rownr == 0. In case we solve the
     dual of the problem we don't want to treat bounds differently */
  if((row == 0) || Solve_dual)
    {
      store(var,  row,  value);
      return;
  ...
  if(Lin_term_count == 2) /* now you can also store the first variable */
	...
	fprintf(stderr, "(var_store) Warning, variable %s has an effective coefficient of 0 on line %d. Ignored.\n", tmp_store.name, yylineno);
...
void store_bounds(void)
{
  if (tmp_store.value != 0)
    ...
    {
      fprintf(stderr, "Error, variable %s has an effective coefficient of 0 in bound on line %d. Ignored.\n", tmp_store.name, yylineno);
      exit(1);
    }
...
/* transport the data from the intermediate structure to the sparse matrix as
   the dual of the original problem, and free the intermediate structure */
void readinput_dual(int *cend,
		    REAL *rh,
		    short *relat,
		    REAL *lowbo,
		    REAL *upbo,
		    ...
		    nstring *names)
{
  int i, j, dual_row, mat_index, orig_nonnuls = Nonnuls;
  rside *rp;
  hashelem *hp;
  column *cp;
  constraint_name *cnp;
  short *orig_relat; /* to store the relational operators of the original
			problem */
  REAL *orig_rh; /* to store the right hand side values of the original
			problem */

  CALLOC(orig_relat, Columns + 1, short);
  CALLOC(orig_rh, Columns + 1, REAL);
  ...
  /* copy the information of the original RHS */
  for (i = Columns; i > 0; i--)
    {
      rp = First_rside;
      orig_relat[i] = rp->relat;
      orig_rh[i] = rp->value;
      First_rside = rp->next;
      free(rp);  /* free memory when data has been read */
    }

  /* this will be terribly confusing. Rows and Columns have already being
     swapped at this point, and refer to the dual problem. However, the rows
     and columns values in the stored information refers to the original
     problem. So, watch out! */

  /* fill names with the column names of the dual, the rownames of the orig */
  for(cnp = First_constraint_name; cnp; cnp = cnp->next)
    strcpy(names[Rows + cnp->row], cnp->name);
  for (i = Rows + 1; i <= Sum; i++)
    if(!*names[i])
      /* Do not override user specified names */
      sprintf(names[i], "%s%d", STD_ROW_NAME_PREFIX, i - Rows);

  /* get the row names of the dual. The first name in the hash table becomes
     row 1 (row zero is the objective function) */
  dual_row = 0;
  strcpy(names[0], "r_0");
  for(j = 0; j < HASHSIZE; j++)
    {
      for(hp = Hash_tab[j]; hp; hp = hp->next)
	{
	  dual_row++;
	  strcpy(names[dual_row], hp->colname);
	}
    }    

  /* create the rhs and the relat of the dual problem */
  dual_row = 0;
  relat[0] = OF;
  for(i = 1; i <= Rows; i++)
    {
      relat[i] = GE; /* all dual rows will be like this */
      rh[i] = 0; /* initialise to 0, some will change in the next loop */
    }
  ...
  for(j = 0; j < HASHSIZE; j++)
	...
	{
	  dual_row++;
	  for(cp = hp->col; cp; cp = cp->next)
	    {
	      if(cp->row == 0) /* this is a OF entry in the original */
		{
		  /* the sign depends on the direction of optimisation */
		  rh[dual_row] = Maximise ? cp->value : - cp->value;
		  Nonnuls--;
		}
	    }
	}
  ...
  /* add the non-zero entries in the RHS of the original to the nonnuls */
  for(i = 1; i <= Columns; i++)
    {
      if(orig_rh[i] != 0) /* only if non-zero */
	{
	  Nonnuls++;
	}
    }

  if(Nonnuls > orig_nonnuls) /* reallocate the arrays depending on it */
    {
      free(Colno);
      free(Mat);
      free(Mat);
  ...
  for(i = 1; i <= Columns; i++)
    {
      /* collect objective function. The coefficients are the RHS values of
	 the original problem. The sign depends on the relational operator of
	 the row in the original and on the direction of optimisation */
      if(orig_rh[i] != 0) /* only if non-zero */
	{
	  if(orig_relat[i] == LE)
	    {
	      (*mat)[mat_index].value = Maximise ? orig_rh[i] : - orig_rh[i];
	    }
	  else if(orig_relat[i] == GE)
	    {
	      (*mat)[mat_index].value = Maximise ? - orig_rh[i] : orig_rh[i];
	    }
	  (*mat)[mat_index].rownr = 0;
	  mat_index++;
	}
     
      /* Go through hash table to see if there are variables in column i */
      dual_row = 0;
      for(j = 0; j < HASHSIZE; j++)
	{
	  for(hp = Hash_tab[j]; hp; hp = hp->next)
	      ...
	      for(cp = hp->col; cp; cp = cp->next)
		{
		  if(cp->row == i)
		    {
		      /* this should depend on the rel. op. of cp->row */
		      if(orig_relat[i] == LE)
			{
			  (*mat)[mat_index].value = cp->value;
			  (*mat)[mat_index].rownr = dual_row;
			}
		      else if(orig_relat[i] == GE)
			{
			  (*mat)[mat_index].value = - cp->value;
  ...
  Maximise = !Maximise; /* swap maximising and minimising */
  ...
  if(Verbose)
    {
      for(i = 0; i <= Columns; i++)
	{
	  fprintf(stderr, "cend[%d] = %d\n", i, cend[i]);
	}
      
      for(i = 0; i < Nonnuls; i++)
	{
	  fprintf(stderr, "mat[%d].value = %lg,", i, (*mat)[i].value);
	  fprintf(stderr, "\tmat[%d].rownr = %d\n", i, (*mat)[i].rownr);
	}
      ...
      for (i = 0; i < Nonnuls; i++)
	{
	  if (i == cend[j])
	    j++;
	  printf("    %-8s  %-8s  %g\n", names[Rows + j],
		 names[(*mat)[i].rownr], (double)(*mat)[i].value);
	}
