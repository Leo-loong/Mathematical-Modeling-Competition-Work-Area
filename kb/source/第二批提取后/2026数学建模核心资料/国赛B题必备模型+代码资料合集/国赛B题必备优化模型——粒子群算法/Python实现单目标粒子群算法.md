...
gbpos = self.cur_pos[i] elif self.mode == 'max':
if self.fitness(self.cur_pos[i]) > self.fitness(self.gbpos):
...
self.cur_spd[i,n] = MAX_SPD[n] elif self.cur_spd[i,n] < MIN_SPD[n]:
self.cur_spd[i,n] = MIN_SPD[n]

# Calculate the position after particle iteration
# Update particle position
self.cur_pos[i] = self.cur_pos[i] + self.cur_spd[i] for n in range(self.pdim):
if self.cur_pos[i,n] > MAX_POS[n]:
self.cur_pos[i,n] = MAX_POS[n] elif self.cur_pos[i,n] < MIN_POS[n]:
...
self.bpos[k] = self.cur_pos[k] elif self.mode == 'max':
if self.fitness(self.cur_pos[k]) > self.fitness(self.bpos[k]):
...
self.gbpos = self.bpos[k] elif self.mode == 'max':
if self.fitness(self.bpos[k]) > self.fitness(self.gbpos):
...
您可能感兴趣的文章:Python编程实现粒子群算法(PSO)详解
