    ...
    %输入: dfk是f(x)在点xk处的梯度, Bk是第k次近似Hesse矩阵,
    %        Ae,hk是线性等式约束的有关参数, Ai,gk是线性不等式约束的有关参数
    %输出: k是迭代次数d,val分别是最优解和最优值, mu,lam是乘子向量
    n=length(dfk);  l=length(hk);  m=length(gk);
    beta=0.5;  sigma=0.2;  epsilon=1.0e-6;  gamma=0.05;
    ...
    while (k<=150)
        ...
        while (i<=20)
            if(l>0&m>0)
                ...
                mu1=mu+beta^i*du;
                lm1=lm+beta^i*dl;
                dh1=dah(ep1,d1,mu1,lm1,dfk,Bk,Ae,hk,Ai,gk);
            end
            if(l==0)
                t1=beta^i; 
