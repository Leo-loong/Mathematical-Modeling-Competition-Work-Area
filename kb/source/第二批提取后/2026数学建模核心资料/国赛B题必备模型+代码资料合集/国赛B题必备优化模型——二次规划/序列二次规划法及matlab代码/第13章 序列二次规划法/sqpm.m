     ...
     %功能: 基于拉格朗日函数Hesse矩阵的SQP方法求解约束优化问题:
     %        min  f(x),
     %         s.t.  h_i(x)=0 (i=1,2,..., l),
     %               g_i(x)>=0 (i=1,2,...,m).
     %输入: x0是初始点, mu0, lam0是乘子向量的初始值
     %输出: k是迭代次数, x,mu,lam分别是近似最优点及相应的乘子向量, val是最优值
     kmax=1000;   %最大迭代次数
     ...
     k=0;
     while(k<kmax)
        [k1,dk,mu,lm,val]=qpsubp(dfk,Bk,Ae,hk,Ai,gk); %求解子问题
        mp1=norm(hk,1)+norm(max(-gk,0),1);
        if(norm(dk,1)<epsilon1) & (mp1<epsilon2)  %检验终止准则
            break;
        end   
        deta=0.05;  %罚参数更新
