...
选择过程是利用解码后求得的各个体适应值大搜索多变量函数的最优解。为简单起见,下面仅以一
小,淘汰一些较差的个体而选出一些比较优良的个体, 个单变量函数为例,来验证所编遗传算法程序的全局
以进行下一步的交叉和变异操作。选择算子的程序如寻优能力。设函数为:y = cos (5x)2sin(3x) + 10 ,x ∈[1
下: 7] ,函数特性如图1 所示。
...
作者简介:
图3   最佳个体的变化情况
由于采用了最优保存策略,所以在图2 中未看到刘国华(19742) ,男,博士生,研究方向为人工智能及其在计算
