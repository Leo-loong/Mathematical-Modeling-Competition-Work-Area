...
for i = 1 : Nind
   if rand(1) < 0.5
       chrom(i,2) = ceil(chrom(i,2));
   else
       chrom(i,2) = floor(chrom(i,2));
   end
end
...
% gaDat.Chrom(i,2)=round(gaDat.Chrom(i,2));
...
for i = 1 : nind
%     c(i)=max(0,17-x(i,2)-ee)+max(0,16-x(i,1)./x(i,3)-ee) + max(0,x(i,1)./x(i,3)-35-ee) + max(0,0.2-x(i,3)-ee)...
%      + max(0,x(i,2)*x(i,3)-30-ee) + max(0,10-x(i,5)) + max(0,13-x(i,6)-ee) + max(0,x(i,5)-15-ee) + max(0,x(i,6)-20-ee)...
%      + max(0,4+0.5*x(i,6)+x(i,1)-x(i,4)-ee) + max(0,-8550-441630./(x(i,2)*x(i,3)*sqrt(x(i,1)))-ee)...
%      + max(0,70980./(x(i,1)*x(i,2)*x(i,3)^2*(0.169+66.66*(1e-4)*x(i,2)-85.4*(1e-6)*x(i,2)^2))-2617-ee)...
%      + max(0,70980./(x(i,1)*x(i,2)*x(i,3)^2*(0.2824+17.7*(1e-4)*x(i,2)-39.4*(1e-6)*x(i,2)^2))-2133-ee)...
%      + max(0,0.01233*x(i,4)^3./(x(i,2)*x(i,3)*x(i,5)^4)-0.003*x(i,4)-ee) + max(0,(10./x(i,5)^3)*(sqrt((29050*x(i,4)./(x(i,2)*x(i,3)))^2+(0.58*27300)^2))-550-ee)...
%      + max(0,(10./x(i,6)^3)*(sqrt((29050*x(i,4)./(x(i,2)*x(i,3)))^2+(0.58*27300)^2))-550-ee);
% c(i)=max(0,x(i,1)^2+x(i,2)^2+x(i,3)^2-5) + max(0,(x(i,4)-3)^2+x(i,5)^2-1) + max(0,x(i,6)-8) + max(0,4-x(i,6));
c(i) = max(0,17-x(i,2)-ee) + max(0,0.7-x(i,1)/(x(i,2)*x(i,3))-ee) + max(0,x(i,1)/(x(i,2)*x(i,3))-1.65-ee) + max(0,2-x(i,2)-ee)...
    + max(0,x(i,2)*x(i,3)-314-ee) + max(0,20-x(i,5)-ee) + max(0,50-x(i,6)-ee) + max(0,x(i,5)-150-ee) + max(0,x(i,6)-200-ee)...
    + max(0,302963.5/(x(i,2)*x(i,3)*sqrt(x(i,1)))-510-ee) + max(0,2555615.782/(x(i,1)*x(i,2)*x(i,3)^2)-150-ee) + max(0,2422173.319/(x(i,1)*x(i,2)*x(i,3)^2)-139.75-ee)...
    + max(0,0.1214*x(i,4)^3/(x(i,2)*x(i,3)*x(i,5)^4)-0.003*x(i,4)-ee) + max(0,10/x(i,5)^3*sqrt((286064*x(i,4)/(x(i,2)*x(i,3)))^2+(0.6*276300)^2)-60-ee)...
    + max(0,10/x(i,6)^3*sqrt((286064*x(i,4)/(x(i,2)*x(i,3)))^2+(0.58*276300)^2)-60-ee);
end
...
% MUTATION ------------------------------------------------
Chrom = mutbga(SelCh,gaDat.FieldD,[gaDat.Pm 1]); % Codificaci髇 Real.
...
[Nind temp] = size(NewChrom);

% for i = 1 : Nind
%     if NewChrom(i,3)<2 && NewChrom(i,3)>=1.5;
%         NewChrom(i,3)=2;
%     elseif NewChrom(i,3)<1.5 && NewChrom(i,3)>=1.25;
%         NewChrom(i,3)=1.5;
%     elseif NewChrom(i,3)<1.25 && NewChrom(i,3)>=1;
%         NewChrom(i,3)=1.25;
%      elseif NewChrom(i,3)<1 && NewChrom(i,3)>=0.8;
%         NewChrom(i,3)=1;
%         elseif NewChrom(i,3)<0.8 && NewChrom(i,3)>=0.7;
%         NewChrom(i,3)=0.8;
%         elseif NewChrom(i,3)<0.7 && NewChrom(i,3)>=0.5;
%         NewChrom(i,3)=0.6;
%         elseif NewChrom(i,3)<0.5 && NewChrom(i,3)>=0.4;
%         NewChrom(i,3)=0.5;
%     else
%         NewChrom(i,3)=0.25;
%     end
% end
for i = 1 : Nind
    if NewChrom(i,3)<10 && NewChrom(i,3)>=8;
        NewChrom(i,3)=10;
    elseif NewChrom(i,3)<8 && NewChrom(i,3)>=6;
        NewChrom(i,3)=8;
    elseif NewChrom(i,3)<6 && NewChrom(i,3)>=5;
        NewChrom(i,3)=6;
     elseif NewChrom(i,3)<5 && NewChrom(i,3)>=4;
        NewChrom(i,3)=5;
        elseif NewChrom(i,3)<4 && NewChrom(i,3)>=3;
        NewChrom(i,3)=4;
        elseif NewChrom(i,3)<3 && NewChrom(i,3)>=2.5;
        NewChrom(i,3)=3;
        elseif NewChrom(i,3)<2.5&& NewChrom(i,3)>=2;
        NewChrom(i,3)=2.5;
    else
        NewChrom(i,3)=2;
    end
...
for i = 1 : Nind
   if rand(1) < 0.5
       NewChrom(i,2) = floor(NewChrom(i,2));
   else
       NewChrom(i,2) = ceil(NewChrom(i,2));
   end
end
