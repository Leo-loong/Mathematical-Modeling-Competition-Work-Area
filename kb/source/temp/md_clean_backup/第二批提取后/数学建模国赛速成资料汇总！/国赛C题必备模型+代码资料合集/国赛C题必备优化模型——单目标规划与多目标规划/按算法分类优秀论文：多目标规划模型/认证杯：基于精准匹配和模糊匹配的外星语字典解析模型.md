...
shift（j） min{s | (P[j1..m]  P[j-s 1..m  s]) &&(P[ j]  P[ j  s])( j  s), P[s 1..m]  P1[..m]( j  s)
以上过程有点抽象，所以我们继续图解。
例2：
...
|来的 2， 样子 号：数|，2，0 ：数|三个值 数|值中选取|取最小值|
|---|---|---|---|---|
...
表 五-12
在 B 处 会同样得到三个值，左边计算后为 3 ，上边计算后为 1 ，在 B 处 由于
...
则计算相似度为：
先取两个字符串长度的最大值 maxLen，用 1-（需要操作数 除 maxLen），得到相
似度。
...
2013.20
[2]张国平， 徐汶东.字符串模式匹配算法的改进 中国石油大学 (华东) 计算机与通信
工程学院 2007.10
