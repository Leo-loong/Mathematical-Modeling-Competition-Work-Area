...
HERE = os.path.dirname(os.path.abspath(__file__))          # .../09_代码与复现/code
SRC = os.path.abspath(os.path.join(HERE, '..'))            # .../09_代码与复现
PKG = os.path.abspath(os.path.join(SRC, '..'))             # .../20_交付包
WS = os.path.abspath(os.path.join(PKG, '..'))              # 工作区根
...
def rd(name):
    """日志在 09_代码与复现/ 根；CSV（q2_e5_S.csv）在 code/ 下 —— 两处都找。"""
    for d in (SRC, HERE):
        p = os.path.join(d, name)
        if os.path.exists(p):
            with io.open(p, encoding='utf-8') as f:
                return f.read()
    raise IOError('未找到 %s' % name)
