...
def _find_root(p, marker='10_赛题', _max=8):
    ...
    ★2026-09-11 修复：原实现为 `PKG = HERE/../..`、`WS = PKG/..`，
    只在"位于交付包 `09_代码与复现/code/` 内"时才成立；一旦从工作区
    `11_建模/11-3_算法与管线/Q2/code/` 运行，就会解析到不存在的
    `11_建模/10_赛题/...` ⟹ **本脚本长期无法产出 `fig_q2_pareto.csv`**
    （F-30 数据缺口的真因）。现改为与 `q2_solver.py` 同款的自适应根定位。
    """
    cur = p
    for _ in range(_max):
