...
@njit(cache=True, fastmath=False)
def _D_nb(C, Tc, d_fac, app3, c_fr):
    c = C
    if c < 1e-9:
        c = 1e-9
    if c_fr > 0.0 and c < c_fr:
        c = c_fr          # ★IN-UQ：低 C 端 D 冻结（C<C_fr 取 D(C_fr)）；c_fr<=0 = 交付口径
    if app3:
...
@njit(cache=True, fastmath=False)
def _faceD4_nb(Carr, Tarr, d_fac, useT, nsamp, app3, c_fr):
    ...
    c_fr>0：采样区间端点与采样点一并做低 C 端冻结钳位（IN-UQ）；c_fr<=0 = 交付口径。
...
def _step_all_nb(N, dxi, hsub, n_sub, R0, hc, km, T, C, t0,
    ...
    for s in range(n_sub):
        ...
        #   if_mode=1：IN-XCHK 对照轨——节点值调和平均（旧口径 _iface_nb），仅对照用
        if if_mode == 1:
            kf = _iface_nb(kk)
        else:
        ...
        for it in range(1, maxit + 1):
            ...
            if mode_flag == 1:
                ...
                Df = np.empty(N - 1 + 1)
                dv = _D_nb(C0f, 28.0, d_fac, app3, c_fr)
                for j in range(N - 1 + 1):
                    Df[j] = dv
            elif mode_flag == 2:
                Df = _faceD4_nb(Cit, Tcur, 1.0, False, 8, app3, c_fr)   # decoupled：仅 C 因子
            else:
                if if_mode == 1:
                    # IN-XCHK 对照轨：节点 D 的调和平均（旧口径）
                    Dn = np.empty(NC)
                    for j in range(NC):
                        Dn[j] = _D_nb(Cit[j], Tcur[j], d_fac, app3, c_fr)
                    Df = _iface_nb(Dn)
                else:
                    Df = _faceD4_nb(Cit, Tcur, d_fac, True, 8, app3, c_fr)  # coupled
            # 中心行
...
def step_imex4(N, dxi, hsub, n_sub, R0, hc, km, T, C, t0,
               ...
               hcv=-1.0, L=0.0, c_fr=-1.0, if_mode=0):
    """推进一个输出步。返回 (T, C, info)。hcv>=0 时覆盖对流系数；L>0 为潜热源项；
    c_fr>0 为低 C 端 D 冻结（IN-UQ）；if_mode=1 为节点值调和平均对照轨（IN-XCHK）。"""
    ...
    Tk, Ck, it, rm = _step_all_nb(
        ...
        int(t_corr), float(hcv), float(L), float(c_fr), int(if_mode))
