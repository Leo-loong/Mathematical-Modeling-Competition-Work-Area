...
% ★ v14：**带圈数字路由**——`①`–`⑳`（U+2460–U+24FF）不在 xeCJK 默认的 CJK 区间内，
%   会被送到西文字体（本件 `\setmainfont{Times New Roman}`）⟹ Times 无此字形，
%   实测产出 **9 处 `Missing character`（①②③④⑤⑥）**。划入 CJK 字符类后由宋体渲染 ⟹ 缺字归零。
%   **仅改字形路由，不改任何字体/字号/样式**。
...
% ★ v23（附录 B 扩录至 B.12 后新暴露的两处缺字，来自 `code/q3_verify_nu.py`／`code/q4_w6_e5.py`
%   注释里的 `≫`／`≪`——DejaVu Sans Mono 无此二字形）：映射到数学字体的 `\gg`／`\ll`，
%   与 v12 同一处置（**不改 `code/` 一个字节**）。
\newunicodechar{≫}{\ensuremath{\gg}}
\newunicodechar{≪}{\ensuremath{\ll}}
% ★ 本轮（附录改 Consolas 后暴露的 4 个缺字）：Consolas **无** ★(U+2605)／⚠(U+26A0)／
%   ⟨(U+27E8)／⟩(U+27E9)。此处按"映射到已有字形"处理（不改 `code/` 与表格内容一个字节）：
%   ★→`\ding{72}`（pifont）、⟨⟩→数学字体的 `\langle`／`\rangle`、⚠→DejaVu Sans Mono（覆盖广）。
\newunicodechar{★}{\ding{72}}
\newunicodechar{⟨}{\ensuremath{\langle}}
\newunicodechar{⟩}{\ensuremath{\rangle}}
% ⚠ 用 `\char"26A0` 输出字形本身（若写成 `{\dejavumono ⚠}` 会因映射再次命中自己 ⟹ 无限递归、
%   TeX capacity exceeded；实测已复现并改用本写法）。
\newunicodechar{⚠}{{\dejavumono\char"26A0}}
% ★ 再补 7 个 Consolas 缺字形（实测 31 处）：数学符号走数学字体，其余走 DejaVu 兜底；
%   下标 t 用 LaTeX 的 `\textsubscript`（保持"下标"语义，不写成普通字母）。
\newunicodechar{∇}{\ensuremath{\nabla}}
\newunicodechar{∈}{\ensuremath{\in}}
\newunicodechar{∝}{\ensuremath{\propto}}
\newunicodechar{≲}{{\dejavumono\char"2272}}
\newunicodechar{≳}{{\dejavumono\char"2273}}
\newunicodechar{▶}{{\dejavumono\char"25B6}}
...
\IfFontExistsTF{DejaVu Sans Mono}{\newfontfamily\lstmonofam{DejaVu Sans Mono}\newfontfamily\dejavumono{DejaVu Sans Mono}}{\let\lstmonofam\ttfamily\newcommand{\dejavumono}{}}
% ============================================================================
% ★ 本轮（用户指令）：附录源程序加**IDE 式语法高亮**（VS Code 配色）
%   · 用 listings **内置的 Python 词法** —— 不需要 `-shell-escape`，比 minted 快且稳
%     （附录 B 有约 150 页代码，minted 会显著拖慢编译并引入外部依赖）。
%   · 配色取 VS Code 常用色：关键字＝蓝、字符串＝暗红、注释＝绿。
%   · **注释不加 `\itshape`**：Consolas／宋体都无斜体字形，加斜体会触发字体替换告警。
%   · **中文注释只上色、不换字体**（仍由 xeCJK 送宋体）。
%   · 行号与具体数字着色**未启用**（会改动代码块版式与行宽；如需可再开）。
% ============================================================================
\definecolor{pykw}{HTML}{0000FF}   % 关键字（if/def/class/return…）
\definecolor{pystr}{HTML}{A31515}  % 字符串
\definecolor{pycmt}{HTML}{008000}  % 注释（含中文注释）
% ---- 识别列表（函数/类名、全大写常量、import 模块别名、数字字面量）----
%   由脚本**从附录 B 实际嵌入的源程序里自动抽取**，生成 `pylists.tex`（勿手改）：
%       python 40_复核\06_偏移性检查\temp\gen_pylists.py
%   代码改动后重跑该脚本即刷新。**该文件缺失时退回下面的基础样式**（仅
%   关键字/字符串/注释着色），故换机也不会编译失败。
\IfFileExists{pylists.tex}{\input{pylists}}{%
...
\newcommand{\srcfile}[1]{%
  ...
  \IfFileExists{code/\srcpath}{\lstinputlisting[style=pycode]{code/\srcpath}}%
...
\lstset{basicstyle=\ttfamily\footnotesize,breaklines=true,frame=single,
        ...
        %   逐字节一致、哈希不变），仅影响渲染。
        literate={⟹}{{\ensuremath{\Longrightarrow}}}1 {✅}{{\ding{51}}}1 {≫}{{\ensuremath{\gg}}}1 {≪}{{\ensuremath{\ll}}}1
        {★}{{\ding{72}}}1 {⚠}{{\dejavumono\char"26A0}}1 {⟨}{{\ensuremath{\langle}}}1 {⟩}{{\ensuremath{\rangle}}}1
        {∇}{{\ensuremath{\nabla}}}1 {∈}{{\ensuremath{\in}}}1 {∝}{{\ensuremath{\propto}}}1
        {≲}{{\dejavumono\char"2272}}1 {≳}{{\dejavumono\char"2273}}1 {▶}{{\dejavumono\char"25B6}}1
        {ₜ}{{\textsubscript{t}}}1}
...
% ★ 本轮（用户指令）：**压缩标题的上下行间距**（段前／段后），用于回收正文页数。
%   背景：正文 9 个 section ＋ 25 个 subsection ＋ 23 个 subsubsection ≈ 57 个标题；
%   按 ctex 默认（section 前 3.5ex／后 2.3ex，次两级 前 3.25ex／后 1.5ex）压缩到
%   下列值，预计回收 500–600 pt（≈0.8 页），且**只动标题周围空白**、不动字号字体。
%   ⚠ 紧凑度是观感取舍：若觉得挤，把 `beforeskip` 回调到 2.0ex／1.8ex 即可（一键回退）。
\ctexset{section={beforeskip=1.5ex plus .2ex minus .2ex, afterskip=1.0ex plus .2ex},
         subsection={beforeskip=1.5ex plus .2ex minus .2ex, afterskip=0.7ex plus .2ex},
         subsubsection={beforeskip=1.4ex plus .2ex minus .2ex, afterskip=0.6ex plus .2ex}}
...
% ============================================================================
% ★ 本轮（试用）：**页眉字体 = 金山云技术体（商免）**
%   · 该字体为**用户级安装**（%LOCALAPPDATA%\Microsoft\Windows\Fonts\Kingsoft_Cloud_Font.ttf），
%     XeLaTeX 的字体库**看不到族名**——实测 `\IfFontExistsTF` 对「金山云技术体」「金山云技术」
%     「Kingsoft Cloud Font」「Kingsoft_Cloud_Font」**全部 not found** ⟹ 改**按文件路径加载**
%     （fontspec 的 `Path=`＋文件名），绕开字体库枚举。
%   · 字体随件放 `fonts/Kingsoft_Cloud_Font.ttf`；**缺该文件时 `\headfont` 退化为空**
%     （即用正文主字体），**仍可编译**，不会因换机而编译失败。
%   · 页眉**结构与位置保持不变**：左＝章号＋章名（`\leftmark`），右＝页码（`\thepage`），
%     **不画**页眉横线（保持现状的"无线"外观）。正文页数因此**不应变化**。
%   · **一键回退**：删掉本区块的两行 `\pagestyle{kyhead}`／改回 `\pagestyle{headings}` 即复原。
% ============================================================================
\IfFileExists{fonts/Kingsoft_Cloud_Font.ttf}{%
  \newfontfamily\headfont[Path=fonts/]{Kingsoft_Cloud_Font.ttf}%      % 西文/数字
  \newCJKfontfamily\headfontcjk[Path=fonts/]{Kingsoft_Cloud_Font.ttf}% % 中文（关键：只定义 \headfont 时中文仍走 SimSun）
}{\newcommand{\headfont}{}\newcommand{\headfontcjk}{}}
\usepackage{fancyhdr}
\fancypagestyle{kyhead}{%
  \fancyhf{}%
  \fancyhead[L]{\headfontcjk\headfont\small\leftmark}%
  % ★ 官方《论文格式规范（2026 年修订稿）》第三条原文：
  %   "论文第三页为摘要专用页……论文**从此页开始编写页码，页码位于页脚中部**，
  %    用阿拉伯数字从"1"开始连续编号。"
  %   ⟹ 页码必须放在**页脚居中**（原实现放在页眉右侧 ✗）。此处把页码从页眉移到页脚中部；
  %   页眉只保留章号＋章名（规范未规定页眉，第八条明确"字体字号等不做统一要求"）。
  % 页脚＝**页码**：不套金山云，落回主字体 **Times New Roman**（KB《AI 论文全维度自查表》
  %   13.2 原文："正文宋体小四，标题黑体，**公式 Times New Roman**，**没有多种字体混用**"
%   ⟹ 页码用主字体 Times：既与公式同族，也**少引入一种字体**（合该条"不混用字体"的要求）。
%   注：KB **未单独规定页码字体**；官方第八条也明确字体字号自定）。
  \fancyfoot[C]{\small\thepage}%
  \renewcommand{\headrulewidth}{0pt}%
  \renewcommand{\footrulewidth}{0pt}%
}
\pagestyle{kyhead}
% ★ 摘要页用的是 `\thispagestyle{plain}`；fancyhdr 的 plain 默认沿用**正文主字体**
%   ⟹ 实测第 1 页页码为 TimesNewRomanPSMT 12.0pt，与第 2 页起的
%   Kingsoft_Cloud_Font 10.9pt **不一致** ✗。此处把 plain 与 kyhead 统一：**页码走主字体 Times**。
