...
% ============================================================================
% ★ 本轮（用户指令）：**仅附录**的"源程序 ＋ 支撑材料文件列表"改用 **Consolas**；
%   **正文一律保持原样**（已核实：正文不使用 \texttt／等宽字体 ⟹ 改动不可能外溢）。
%   ① `\renewfontfamily\lstmonofam{Consolas}` ⟹ 附录 B 的 `\srcfile` 代码块换字体
%      （`\srcfile` 内部正是用 `\lstmonofam` 作 `basicstyle`）；
%   ② `\setmonofont{Consolas}` ⟹ 附录 A／C 文件名清单里的 `\texttt` 换字体；
%   ③ **中文不受影响**：中文仍由 xeCJK 送宋体（Consolas 不含 CJK，代码注释里的中文照旧渲染）；
%   ④ Consolas 缺的字形（★／⟹／✅ 等）由 `preamble.tex` 的 `literate` 映射兜底；
%   ⑤ 本文件是 `main.tex` 的**最后一个** `\input` ⟹ 上述改动不会影响正文任何页面；
%   ⑥ 回退：删掉本区块即恢复到 DejaVu Sans Mono（附录 B）＋ 宋体等宽（附录 A）。
% ============================================================================
\IfFontExistsTF{Consolas}{%
  \renewfontfamily\lstmonofam{Consolas}%
  \setmonofont{Consolas}%
}{}
\appendix
附录按官方《论文格式规范》第五、十一条的要求给出两部分内容：\textbf{支撑材料文件列表}与\textbf{源程序}；另附关键中间结果的索引说明。源程序按“\textbf{交付核、主流程与全部检验脚本全文 ＋ 其余溯源脚本清单}”两段收录（附录 B：B.1–B.7 为关键程序，正文\textbf{全文嵌入}；B.8–B.13 为其余源程序，只列\textbf{文件名清单}），全部源程序的全文随\textbf{支撑材料}提交，并在表~\ref{tab:A-1} 中逐项登记。为免歧义，本附录表内所称含水率均指干基含水率。
...
\settabw{8}
\begin{longtable}{>{\centering\arraybackslash}m{\dimexpr 0.071\tblw\relax}>{\raggedright\arraybackslash}m{\dimexpr 0.236\tblw\relax}>{\centering\arraybackslash}m{\dimexpr 0.108\tblw\relax}>{\raggedright\arraybackslash}m{\dimexpr 0.585\tblw\relax}}
  ...
  \bottomrule
  \endlastfoot
  J1 & \texttt{q1\_\allowbreak{}run\_\allowbreak{}log.\allowbreak{}txt} & 2.2 KB & 口径：现行；主力求解日志：中心双路比对、单遍收集、程序化断言通过 \\
  J2 & \texttt{q1\_\allowbreak{}e1\_\allowbreak{}log.\allowbreak{}txt} & 2.3 KB & 口径：现行；收敛性：空间网格与时间收敛（温度、含水率分别） \\
  J3 & \texttt{q1\_\allowbreak{}e2\_\allowbreak{}log.\allowbreak{}txt} & 6.6 KB & 口径：现行；解析对拍：圆柱级数解（常边界与分段常边界） \\
  J4 & \texttt{q1\_\allowbreak{}e3\_\allowbreak{}log.\allowbreak{}txt} & 6.9 KB & 口径：现行；独立实现互验：显式格式与主力实现 \\
  J5 & \texttt{q1\_\allowbreak{}e4\_\allowbreak{}log.\allowbreak{}txt} & 2.2 KB & 口径：现行；物理看门狗：13 项全部通过并含注错演示 \\
  J6 & \texttt{q1\_\allowbreak{}e5\_\allowbreak{}log.\allowbreak{}txt} & 7.8 KB & 口径：现行；灵敏度：扩散系数、换热系数、传质系数各 ±20\%，初始含水率 ±5\%，初始温度 ±1 ℃ \\
  J7 & \texttt{q1\_\allowbreak{}e6\_\allowbreak{}log.\allowbreak{}txt} & 2.2 KB & 口径：现行；预处理对照（温度侧无内部子步） \\
  J8 & \texttt{conservation\_\allowbreak{}log.\allowbreak{}txt} & 1.3 KB & 口径：现行；守恒性核算：质量与能量区域积分闭合 \\
  J9 & \texttt{q1\_\allowbreak{}consistency\_\allowbreak{}log.\allowbreak{}txt} & 0.3 KB & 口径：现行；同口径幂等性核验：重跑主力结果与交付文件逐值一致，用于关闭“论文数字与程序输出不符”的风险 \\
  J12 & \texttt{q1\_\allowbreak{}upgrade\_\allowbreak{}diff\_\allowbreak{}log.\allowbreak{}txt} & 0.7 KB & 口径：对照轨（非缺陷）；升级前后对照（跨口径）：温度最大逐值差 1.0×10\textsuperscript{-4} ℃（14189 格）、含水率逐位一致，证明升级只动温度侧 \\
  J10 & \texttt{dq\_\allowbreak{}check\_\allowbreak{}log.\allowbreak{}txt} & 3.5 KB & 口径：与时间格式无关；数据质量核查（点位数、步长、缺失、离群） \\
  J11 & \texttt{q1\_\allowbreak{}diag\_\allowbreak{}log.\allowbreak{}txt} & 0.7 KB & 口径：与交付口径无关；诊断（非真值）：保留旧口径以复现早期缺陷，故重跑后与旧日志一致，属自洽复现 \\
\end{longtable}
...
\settabw{8}
\begin{longtable}{>{\centering\arraybackslash}m{\dimexpr 0.068\tblw\relax}>{\raggedright\arraybackslash}m{\dimexpr 0.433\tblw\relax}>{\raggedright\arraybackslash}m{\dimexpr 0.162\tblw\relax}>{\raggedright\arraybackslash}m{\dimexpr 0.337\tblw\relax}}
  ...
  \bottomrule
  \endlastfoot
  32 & \texttt{code/\allowbreak{}q3\_\allowbreak{}core.\allowbreak{}py} & 合计 8.6 KB（1 件缺） & 口径：—；第三问交付求解核（环境预查表与子步循环下沉） \\
  33 & \texttt{code/\allowbreak{}q3\_\allowbreak{}solver.\allowbreak{}py} & 合计 12.1 KB（1 件缺） & 口径：—；第三问主力求解（事件驱动、60 s 输出、子步级二分定位、熔断 120 h、六项断言） \\
  34 & \texttt{code/\allowbreak{}q3\_\allowbreak{}pretest\_\allowbreak{}bench.\allowbreak{}py} & 合计 6.7 KB（1 件缺） & 口径：—；冒烟微基准（成本结构分解；不产交付物） \\
  35 & \texttt{code/\allowbreak{}q3\_\allowbreak{}w2\_\allowbreak{}convergence.\allowbreak{}py} & 合计 5.6 KB（1 件缺） & 口径：—；检验：长时程收敛性（空间四档与时间三档） \\
  36 & \texttt{code/\allowbreak{}q3\_\allowbreak{}grid\_\allowbreak{}experiment.\allowbreak{}py} & 合计 4.6 KB（1 件缺） & 口径：—；检验：网格裁决实验（四档加密） \\
  37 & \texttt{code/\allowbreak{}q3\_\allowbreak{}conservation.\allowbreak{}py} & 合计 4.6 KB（1 件缺） & 口径：—；检验：守恒性核算（质量区域积分闭合） \\
  38 & \texttt{code/\allowbreak{}q3\_\allowbreak{}postcheck.\allowbreak{}py} & 合计 4.4 KB（1 件缺） & 口径：—；后置检验：结果表生成、与第二问重叠段一致性、解析量级对照 \\
  39 & \texttt{code/\allowbreak{}q3\_\allowbreak{}e5\_\allowbreak{}sensitivity.\allowbreak{}py} & 合计 6.4 KB（1 件缺） & 口径：—；检验：灵敏度（烘干时长对五个参数，10 算例并行） \\
  40 & \texttt{code/\allowbreak{}q3\_\allowbreak{}core\_\allowbreak{}nu.\allowbreak{}py} & 合计 14.0 KB（1 件缺） & 口径：—；复检：非均匀（表面加密）网格求解核；等距网格上与交付核逐点一致 \\
  41 & \texttt{code/\allowbreak{}q3\_\allowbreak{}verify\_\allowbreak{}nu.\allowbreak{}py}／\texttt{q3\_\allowbreak{}verify\_\allowbreak{}grid.\allowbreak{}py}／\texttt{q3\_\allowbreak{}verify\_\allowbreak{}opt.\allowbreak{}py} & 合计 18.6 KB（1 件缺） & 口径：—；复检：非均匀核验证、细网格收尾验证、加速等价性校验 \\
  42 & \texttt{code/\allowbreak{}q3\_\allowbreak{}probe\_\allowbreak{}crit.\allowbreak{}py}／\texttt{q3\_\allowbreak{}probe\_\allowbreak{}dcut.\allowbreak{}py}／\texttt{q3\_\allowbreak{}probe\_\allowbreak{}grid.\allowbreak{}py} & 合计 18.2 KB（1 件缺） & 口径：—；复检：判据敏感度、低含水率端扩散系数冻结对照、烘干时长网格裁决 \\
  43 & \texttt{code/\allowbreak{}q3\_\allowbreak{}diag\_\allowbreak{}mech.\allowbreak{}py}／\texttt{q3\_\allowbreak{}diag\_\allowbreak{}iface.\allowbreak{}py} & 合计 18.8 KB（1 件缺） & 口径：—；界面口径诊断：常数扩散系数解析对拍、界面取法单变量对照（修正的直接依据） \\
  44 & \texttt{code/\allowbreak{}q3\_\allowbreak{}impact\_\allowbreak{}check.\allowbreak{}py}／\texttt{q3\_\allowbreak{}surface\_\allowbreak{}theory.\allowbreak{}py}／\texttt{q3\_\allowbreak{}diag\_\allowbreak{}lowC.\allowbreak{}py}／\texttt{q3\_\allowbreak{}diag\_\allowbreak{}surface.\allowbreak{}py}／\texttt{q3\_\allowbreak{}check\_\allowbreak{}maxloc.\allowbreak{}py} & 合计 19.1 KB（1 件缺） & 口径：—；只读分析与诊断（不参与交付数值） \\
  45 & \texttt{code/\allowbreak{}q3\_\allowbreak{}audit\_\allowbreak{}consistency.\allowbreak{}py}／\texttt{q3\_\allowbreak{}audit\_\allowbreak{}tables.\allowbreak{}py} & 合计 12.9 KB（1 件缺） & 口径：—；只读审计：口径一致性扫描、结果表与结果文件逐值核对 \\
...
\settabw{8}
\begin{longtable}{>{\centering\arraybackslash}m{\dimexpr 0.099\tblw\relax}>{\raggedright\arraybackslash}m{\dimexpr 0.228\tblw\relax}>{\centering\arraybackslash}m{\dimexpr 0.099\tblw\relax}>{\raggedright\arraybackslash}m{\dimexpr 0.574\tblw\relax}}
  ...
  \bottomrule
  \endlastfoot
  Q2-L1 & \texttt{q2\_\allowbreak{}run\_\allowbreak{}log.\allowbreak{}txt} & 2.0 KB & 口径：现行；交付运行日志：交付核、交替推进、10800 步、六项断言全部通过；关键值与口径登记逐值一致 \\
  Q2-L2 & \texttt{q2\_\allowbreak{}checks\_\allowbreak{}log.\allowbreak{}txt} & 2.1 KB & 口径：现行；独立实现互验与守恒核算的合并日志；以本文件为准 \\
  Q2-L3 & \texttt{q2\_\allowbreak{}e3b\_\allowbreak{}log.\allowbreak{}txt} & 1.8 KB & 口径：现行；强制解耦退化检验（单向耦合下改换热系数，含水率场变化量级） \\
  Q2-L4 & \texttt{q2\_\allowbreak{}e5\_\allowbreak{}log.\allowbreak{}txt} & 2.1 KB & 口径：现行；单因素灵敏度（含初始含水率补检） \\
  Q2-L5 & \texttt{q2\_\allowbreak{}e5b\_\allowbreak{}log.\allowbreak{}txt} & 4.6 KB & 口径：现行；全局灵敏度（筛选法与方差分解法） \\
  Q2-L6 & \texttt{q2\_\allowbreak{}e7\_\allowbreak{}log.\allowbreak{}txt} & 1.1 KB & 口径：旧版留痕；守恒核算的旧口径输出（恒定边界），非交付口径；其价值在发现过程：首次暴露“变物性下含热容的温度积分不是守恒量”，现行结论以合并日志为准 \\
  Q2-L7 & \texttt{q2\_\allowbreak{}e8\_\allowbreak{}log.\allowbreak{}txt} & 2.1 KB & 口径：现行；潜热对照（计入潜热后温度系统性偏低 0.074–0.077 ℃） \\
  Q2-L8 & \texttt{q2\_\allowbreak{}e9\_\allowbreak{}log.\allowbreak{}txt} & 1.8 KB & 口径：现行；自适应步长（墙钟由 47.6 s 降至 9.9 s） \\
  Q2-L9 & \texttt{q2\_\allowbreak{}diag\_\allowbreak{}picard\_\allowbreak{}log.\allowbreak{}txt} & 1.1 KB & 口径：现行（诊断）；内层迭代上限对照诊断：上限取 8 时每子步跑满；上限取 120 且容限收紧时平均 16.362 次每子步 \\
  Q2-L10 & \texttt{q2\_\allowbreak{}opt\_\allowbreak{}verify\_\allowbreak{}log.\allowbreak{}txt} & 1.3 KB & 口径：现行；两个优化核的逐点一致性核对（差异不超过 1.8×10\textsuperscript{-11}） \\
  Q2-L11 & \texttt{w2\_\allowbreak{}foreground\_\allowbreak{}log.\allowbreak{}txt} & 4.1 KB & 口径：现行（预试验）；空间与时间收敛预试验（确定空间步长与内部子步） \\
  Q2-L12 & \texttt{q2\_\allowbreak{}run\_\allowbreak{}orig\_\allowbreak{}log.\allowbreak{}txt} & 2.2 KB & 口径：对照轨（明示）；优化前基线核的运行日志（输出与交付件隔离） \\
  Q2-L13 & \texttt{q2\_\allowbreak{}e1\_\allowbreak{}log\_\allowbreak{}废弃\_\allowbreak{}早期未收敛.\allowbreak{}txt} & 0.5 KB & 口径：废弃；早期外层迭代未收敛阶段留痕；不代表现行口径 \\
  Q2-L14 & \texttt{q2\_\allowbreak{}e3\_\allowbreak{}log\_\allowbreak{}旧版对照.\allowbreak{}txt} & 0.7 KB & 口径：旧版对照；旧编号体系下的对照文本 \\
  Q2-L15 & \texttt{q2\_\allowbreak{}e5\_\allowbreak{}log\_\allowbreak{}旧版stdout.\allowbreak{}txt} & 1.7 KB & 口径：旧版；串行版标准输出留痕 \\
  Q2-L16 & \texttt{q2\_\allowbreak{}e5\_\allowbreak{}log\_\allowbreak{}旧编号残留.\allowbreak{}txt} & 1.8 KB & 口径：旧编号；编号统一前的残留 \\
\end{longtable}
...
\begin{longtable}{>{\centering\arraybackslash}m{\dimexpr 0.143\tblw\relax}>{\raggedright\arraybackslash}m{\dimexpr 0.620\tblw\relax}>{\centering\arraybackslash}m{\dimexpr 0.237\tblw\relax}}
  \caption{（续）　类别 1.3i　未进论文正文的中间结果图件（\texttt{figures/} 目录下，40 张）}\label{tab:A-1-l}\\
  \toprule
    \# & 文件 & 大小 \\
  \midrule
  \endfirsthead
...
\section{源程序}
本附录收录建模所用的源程序：\textbf{公共模块、四问求解的交付核与主流程，以及全部检验脚本}（B.1–B.12）\textbf{全文嵌入}；\textbf{其余溯源脚本}（B.13）\textbf{只列文件名清单}，其全文随\textbf{支撑材料}提交，并按官方第十一条在表~\ref{tab:A-1} 中逐项登记。全篇按“通用模块、分问求解、检验脚本”的顺序编排；附录中不描述运行环境与硬件信息，只保留程序本身及其接口说明。
...
代码一致性声明：附录所列源程序与实际生成四个结果文件的程序同一版本；结果文件与论文各结果表（共六张）逐格一致。按官方要求，源程序缺失、不能运行或运行结果与论文不符者可能被取消评奖资格；故本附录收录的 B.1–B.12 \textbf{不做删节}（仅按上述顺序重组），其余源程序\textbf{不裁剪、文件名列全}（全文随支撑材料提交）。
...
\srcfile{q1_e1_gci.py}
\srcfile{q2_w2_foreground.py}
\srcfile{q3_w2_convergence.py}
\srcfile{q3_grid_experiment.py}
\srcfile{q4_w6_e15.py}

\noindent\textbf{B.9\quad 检验脚本：解析对拍}
\srcfile{q1_e2_series.py}
\srcfile{q2_e2_frozen.py}
\srcfile{q4_w6_e2.py}

\noindent\textbf{B.10\quad 检验脚本：独立实现互验}
\srcfile{q1_e3_explicit.py}
\srcfile{q2_checks.py}
\srcfile{q3_verify_nu.py}
\srcfile{q3_verify_grid.py}
\srcfile{q3_verify_opt.py}
\srcfile{q4_core_fixed.py}

\noindent\textbf{B.11\quad 检验脚本：守恒核算}
\srcfile{q1_conservation.py}
\srcfile{q2_checks.py}
\srcfile{q3_conservation.py}
\srcfile{q4_w6_e78b.py}

\noindent\textbf{B.12\quad 检验脚本：灵敏度分析}
\srcfile{q1_e5_sensitivity.py}
\srcfile{q2_e5_sensitivity.py}
\srcfile{q2_e5b_global.py}
\srcfile{q3_e5_sensitivity.py}
\srcfile{q4_w6_e5.py}
\srcfile{innov/q4_INV3_global.py}
\srcfile{innov/q4_INV6_sobol.py}
