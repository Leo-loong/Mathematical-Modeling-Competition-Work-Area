...
\begin{table}[htbp]
  ...
  \begin{tabular}{>{\centering\arraybackslash}m{\dimexpr 0.260\tblw\relax}>{\raggedright\arraybackslash}m{\dimexpr 0.540\tblw\relax}>{\centering\arraybackslash}m{\dimexpr 0.200\tblw\relax}}
    \toprule
    符号 & 含义 & 单位 \\
    \midrule
    \multicolumn{3}{c}{\bfseries 自变量与几何} \\
    \cmidrule(lr){1-3}
    $t,\ r$ & 时间；到药材中心的距离（径向坐标） & s、cm \\
    $R_0,\ L$ & 药材初始半径、长度 & cm \\
    $R(t),\ \xi$ & 药材半径（随时间变化，仅问题四）；物料坐标 $r/R(t)$ & cm、无量纲 \\
    \multicolumn{3}{c}{\bfseries 场量与初值} \\
    \cmidrule(lr){1-3}
    ...
    \multicolumn{3}{c}{\bfseries 数值参数与无量纲数} \\
    \cmidrule(lr){1-3}
