# -*- coding: utf-8 -*-
"""
A 题 Q2 求解核 · 潜热对照变体（E8 / RK-2）
======================================================
本文件由 `q2_core_c.py` 派生，**唯一差异**是：在**表面热平衡**中加入
**水分蒸发吸热项**（H3 假设的对照实验）。

  原（基线，忽略蒸发潜热）：
      -k ∂_rT|_R = h [T∞(t) - T_R]
  本变体（计入蒸发潜热）：
      -k ∂_rT|_R = h [T∞(t) - T_R] - L · k_m [C_R - C∞(t)]

  其中 L = 2.26e3 kJ/kg = 2.26e6 J/kg（水的汽化潜热，题面/常用值），
  末项为**表面水分蒸发带走的热流**（水分流出时 C_R > C∞ ⟹ 该项为负 ⟹ 表面被"夺热"）。

  ⚠ 该项按**显式**处理（用当前时刻的 C_R），即把潜热作为已知热汇并入右端 ——
    与"物性滞后一步"的处理层级一致（滞后量 ∝ 内部步长），**不改变格式稳定性**。

  ⚠ 派生说明：除本项外，离散公式、迭代流程、物性表达式、追赶法均与 `q2_core_c.py` **逐字相同**。
...
# ★潜热变体专用：蒸发潜热（J/kg）。置 0 即退回【不计潜热】的基线行为。
qc_L_LATENT = 2.26e6
...
def rhs_T_var(N, dr, dt, R0, h, rho, cp, Told, Tinf, out=None,
               Cold=None, cinf=0.0, L_latent=0.0, km=None):
    """右端：固定取上一时层解 Told

    ★潜热变体：给出 `Cold` 时，在**表面行 d[N]** 追加蒸发吸热项
        - L · k_m · R0 · (C_N - C∞)
    该项按显式处理（用当前 C），作为已知热汇并入右端。
    """
    ...
    if _HAS_NUMBA:
        ...
        VN = dr * (R0 - dr / 4.0) / 2.0
        d[N] = rcT[N] * VN / dt + h * R0 * Tinf
    if Cold is not None and L_latent != 0.0:
        _km = DEF2['km'] if km is None else km
        d[N] += -L_latent * _km * R0 * (float(Cold[N]) - float(cinf))
    return d


def _rhs_T_var_orig(N, dr, dt, R0, h, rho, cp, Told, Tinf, out=None):
    """（保留原签名的最小实现，供对照调用）"""
    d = np.zeros(N + 1) if out is None else out
    if _HAS_NUMBA:
        _rhs_T_nb(N, dr, dt, R0, h,
...
def step_imex(N, dr, h, n_sub, R0, hc, km, T, C, Tenv_fn, Cenv_fn, t0,
    ...
    for s in range(n_sub):
        ...
        d = rhs_T_var(N, dr, h, R0, hc, rho, cp, Tcur, tinf, out=dT,
                      Cold=Ccur, cinf=cinf, L_latent=qc_L_LATENT)   # ★潜热项
        Tcur = thomas(a, b, c, d)
