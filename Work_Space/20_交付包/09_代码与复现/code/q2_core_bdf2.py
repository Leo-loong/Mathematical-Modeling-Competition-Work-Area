# -*- coding: utf-8 -*-
"""
A 题 Q2 求解核 · BDF2 变体（O6：二阶时间精度）
======================================================
本文件由 `q2_core_c.py` 派生，**唯一差异**是时间推进格式：

  · 基线：**后向欧拉**（一阶）  (u^{n+1}-u^n)/h = F(u^{n+1})
  · 本变体：**BDF2**（二阶）    (3u^{n+1}-4u^n+u^{n-1})/(2h) = F(u^{n+1})

实现方式（**不改任何装配／右端表达式**）——在调用层做等效替换：
  · 装配用 h' = h/1.5        ⟹ 得到 1.5/h 的系数（＝3/(2h)）✓
  · 右端用 u_eff = (4u^n - u^{n-1})/3，仍配 h' ⟹ (4u^n-u^{n-1})/(2h) ✓
  · 第一步无 u^{n-1} ⟹ 自动退化为后向欧拉（标准启动步）

其余（物性、IMEX 交替推进、Picard、追赶法、边界）与 `q2_core_c.py` **逐字相同**。
...
def step_imex(N, dr, h, n_sub, R0, hc, km, T, C, Tenv_fn, Cenv_fn, t0,
              omega=0.7, tol=1e-10, maxit=30, mode='coupled', bdf2=False):
    ...
    Tprev_dt, Cprev_dt = T.copy(), C.copy()      # ★BDF2：内部步的 n-1 层
    inner_tot = 0
    res_max = 0.0
    # 预分配缓冲（每输出步一次）
    ...
    for s in range(n_sub):
        ...
        # ② T（全隐式／BDF2，物性冻结）
        #   BDF2 等效：h' = h/1.5，右端用 (4T^n - T^{n-1})/3
        if bdf2 and s > 0:
            h_eff = h / 1.5
            T_eff = (4.0 * Tcur - Tprev_dt) / 3.0
        else:
            h_eff = h
            T_eff = Tcur
        a, b, c = assemble_T_var(N, dr, h_eff, R0, hc, rho, cp, kk, out=bufT)
        d = rhs_T_var(N, dr, h_eff, R0, hc, rho, cp, T_eff, tinf, out=dT)
        Tnew = thomas(a, b, c, d)
        Tprev_dt = Tcur
        Tcur = Tnew
        ...
        #   BDF2 等效：h' = h/1.5，右端用 (4C^n - C^{n-1})/3
        if bdf2 and s > 0:
            hC_eff = h / 1.5
            C_eff = (4.0 * Ccur - Cprev_dt) / 3.0
        else:
            hC_eff = h
            C_eff = Ccur
        base = rhs_C_var(N, dr, hC_eff, R0, km, C_eff, cinf)   # 每子步更新 ⟹ 不复用缓冲
        Cit = Ccur.copy()
        for it in range(1, maxit + 1):
            if mode == 'frozen':
                Dn = Dbase
                ...
                Dn = D_of(Cit, Tcur)
            a, b, c = assemble_C_var(N, dr, hC_eff, iface(Dn), R0, km, out=bufC)
            Ctry = thomas(a, b, c, base)
            Cnew = omega * Ctry + (1.0 - omega) * Cit
            rr = float(np.max(np.abs(Cnew - Cit))) / max(1.0, float(np.max(np.abs(Cnew))))
            Cit = Cnew
...
def run_q2(N=80, dr=2.5e-4, dt_out=1.0, nsteps=10800,
           ...
           mode='coupled', scheme='imex', lag_props=True, bdf2=False,
    ...
    for n in range(1, nsteps + 1):
        if scheme == 'imex':
            T, C, info = step_imex(N, dr, hh, nsub, R0, h, km, T, C,
                                   ...
                                   omega, tol, maxit, mode, bdf2=bdf2)
