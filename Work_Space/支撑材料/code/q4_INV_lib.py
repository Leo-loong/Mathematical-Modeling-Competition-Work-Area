...
def solve_to_dry(N, n_sub, t_corr=1, d_fac=1.0, hcv=-1.0, kmv=-1.0,
                 ...
                 verbose=False, tag='', c_fr=-1.0, if_mode=0, C_crit=0.15,
                 fixR=False):
    """Event-driven solve to t_dry with substep bisection; optional history.
    innov 扩展：c_fr>0 低 C 端 D 冻结（IN-UQ）；if_mode=1 调和平均对照轨（IN-XCHK）；
    C_crit 可变判据（IN-CRIT）。"""
    import time
    import q4_core_INV as qc
    dxi = 1.0 / N
    DT = 1.0
    hsub = DT / n_sub
    ...
    while k < max_steps:
        ...
        T, C, info = qc.step_imex4(N, dxi, hsub, n_sub, qc.DEF4['R0'], hc, km,
                                   ...
                                   c_fr=c_fr, if_mode=if_mode, fixR=fixR)
